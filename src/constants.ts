export const ipfsPrefix = 'ipfs://';
export const tokenContractAddress = 'KT1D1hhh4aKTLt79iu4q1M8bfHsUR9cpUKds';
export const voterMoneyPoolContractAddress = 'KT1StnQpS86BUw8gjLNU2aVw6qgeuP5szEe7';
export const auctionHouseContractAddress = 'KT19ubT4oVE4L4KatHE1WJbPb481fXaumei1';
export const theVoteContractAddress = 'KT1DcoAaqwJSXTNcxjUPhkFuW6BJmJL6TG3V';
export const sprayContractAddress = 'KT1KL4jmirDvMXcyYQZoF8TrjwZQwyn4V4bH';
export const bankContractAddress = 'KT1VddB9LRnD5reNo2bugLikcxxQ1cHHTFjG';

export const tokenMetadataIPFSHASH = 'QmchAqaHk7eBDxHEYK6QyekoBW9QFVuXN8qnft3w3PVArG';
export const sprayMetadataIPFSHASH = 'QmSngGsY37p91y6Z3oHZmAtK3VCqrz3KJWVSmkyEjgUBWt';
export const voterMoneyPoolMetadataIPFSHASH = 'Qme5PQCgnXWA332yuLSyDdknM4wwBjtaQKxMLnimWY6yA8';
export const auctionHouseMetadataIPFSHASH = 'QmXqb1KFmP3TbgD2JvartMiY5EmEK1RW61ybTD673dPzA6';
export const voteMetadataIPFSHASH = 'Qmbzz5JKi8Zz3NNuk2yh4V7Epkc1Ggrea9nYArqaqwd5h4';
